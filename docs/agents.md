...
Unavailable models, rejected/malformed RPC responses, admission timeouts, or a remaining model mismatch fail the run without sending its task. The admission window is 90s by default and can be widened per host with `PI_FABRIC_MODEL_ADMISSION_TIMEOUT_MS` (milliseconds; invalid values fall back to the default), still clamped by the run timeout. Queued controls wait for admission too. Fabric does not substitute an MRU model or silently fall back. Standalone `AgentManager` callers can also supply a unique exact bare model ID; ambiguous IDs must be provider-qualified.
